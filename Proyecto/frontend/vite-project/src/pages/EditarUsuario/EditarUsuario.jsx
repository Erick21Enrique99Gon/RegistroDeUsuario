...
function DetalleUsuarioConPasaportes() {
  ...
  const handleCheckboxChange = async () => {
    ...
    try {
      ...
      setUsuario(prev => ({
        ...prev,
        habilitado: !prev.habilitado
      }));

      console.log('Estado actualizado:', !usuario.habilitado);
    } catch (error) {
          ...
          <div className="edit-user-status-row">
            <div className="status-display">
              <label className="status-label">
                Estado:
                <span className={`status-text ${usuario.habilitado ? 'habilitado' : 'deshabilitado'}`}>
                  {usuario.habilitado ? "Usuario habilitado" : "Usuario deshabilitado"} </span>
              </label> </div>

            <button className={`toggle-btn ${usuario.habilitado ? 'active' : 'inactive'}`}
              onClick={handleCheckboxChange} aria-label={usuario.habilitado ? "Deshabilitar usuario" : "Habilitar usuario"} >
              {usuario.habilitado ? 'Deshabilitar' : 'Habilitar'} </button> </div>
